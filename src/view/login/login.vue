...
export default {
  ...
  methods: {
    ...
    handleSubmit ({ username, password, dynamic }) {
      const c_url = this.$route.params.c_url ? this.$route.params.c_url : ''
      this.handleLogin({ username, password, dynamic, c_url}).then(res => {
        if (res.code === 0) {
          this.$Message.success(`${res.msg}`)
          if (res.c_url) {
            window.location.href = res.c_url
          }
          else{
          ...
          this.modalTitle = '根据邮箱注册用户'
          // 邮箱认证通过，请根据邮箱完善用户信息
          }  else if (res.code === -6) {
          this.mail = res.email ? res.email : null
          ...
          this.modalVisible = true
          this.modalTitle = '根据LDAP登录信息注册用户'
          // LDAP认证通过，请完善用户信息
        } else {
          this.$Message.error({
            content: `${res.msg}`,
