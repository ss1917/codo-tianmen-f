        ...
        <Layout>
            <Layout>
                ...
                <Sider hide-trigger :style="{background: '#fff'}">
                  <Menu active-name="WEBSITE" @on-select="handleTabs">
                    <template v-for="(item, index) in tabs">
                      <MenuItem :name=item.name :key="`${_uid}_${index}`">
                        <Icon :type="item.icon" />
                        {{item.label}}
                    </MenuItem>
                    </template>
                      ...
                      <Row :gutter="16">
                        ...
                        <Col span="10">
                          <Alert  v-if="theTabName === 'WEBSITE'">
                            ...
                            <h5>API地址：默认为：window.location.host/api/ 根据实际情况自己调整</h5><br>
                          ...
                          <h5>用户登录DN  ou=opendevops,dc=opendevops,dc=cn</h5>
                          <p>这里是设置用来登录codo的组织单元, 比如我要用某个组织的用户来登录codo</p><br>

                          <h5>用户过滤器</h5>
                          <p>这里是设置认证用户的信息, 系统会使用这个用户去校验ldap的信息是否正确 一般使用cn 或者 sAMAccountName</p><br>

                          <h5> 启动LDAP认证</h5>
                          <p> 如果需要使用LDAP登录, 则必选</p>
                          </Alert>
...
<script>
import { getSysconfig, operationSysconfig, CheckSysconfig } from '@/api/app/sys-conf'
...
export default {
  ...
  data () {
    return {
      ...
      API_GW_URL: "",
      CONFIG_DATA: {
        WEBSITE_API_GW_URL: this.API_GW_URL, // 默认API 地址
  ...
  // },
  created () {
    this.CONFIG_DATA.WEBSITE_API_GW_URL = `${window.location.host}/api/`
    this.getSttings('all')
    setTimeout(() => {
      this.handleTabs('WEBSITE')
    }, 500)
