<template>
  <Drawer v-model="modalData.show" :title="modalData.title" style="background-color: #f8f8f9" width="750px;" draggable :footer-hide=true>
    ...
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100" :label-colon=true>
      ...
      <FormItem label="组件名称" prop="component_name">
        <Input v-model="formValidate.component_name" maxlength="80" type="text" placeholder="请输入组件名称..."></Input>
      </FormItem>
      <!-- <FormItem label="服务编码" >
        <Input v-model="formValidate.app_code" maxlength="80" type="text" placeholder="请输入组件名称..."></Input>
      </FormItem> -->
...
export default {
  ...
  data () {
    return {
      btnLoading: false,
      ruleValidate: {
        component_name: [{ required: true, message: '前端组件名称不能为空', trigger: 'blur' }],
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          setTimeout(() => {
            this.$emit('handle-submit-component', this.formValidate, this.modalData.action)
