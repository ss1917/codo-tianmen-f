    ...
    <Drawer v-model="modalData.show" @on-close='handleCancel' :title="modalData.title" width="750px;" draggable :footer-hide=true>
      ...
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120" :label-colon=true>
        ...
        <FormItem label="手机号码" prop="tel">
          <Input v-model="formValidate.tel" maxlength="11" type="text"></Input>
        </FormItem>
...
export default {
  ...
  methods: {
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          setTimeout(() => {
            this.$emit('handle-submit-user', this.formValidate, this.modalData.action)
    ...
    handleSubmitExec (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.btnLoading = true
          setTimeout(() => {
            operationCommonJobs(this.formValidate, this.modalData.action).then(res => {
              if (res.data.code == 0) {
